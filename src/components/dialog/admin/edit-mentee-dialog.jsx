...
import { useUpdateMenteeMutation } from "@/store/api/mentee.api"
...
export default function AdminEditMenteeDialog({
  open = false,
  onOpenChange,
  onClose,
  mentee,
  ...
  const [updateMentee, { isLoading: isLoadingUpdateMentee }] =
  ...
  useEffect(() => {
    form.reset({
      ...
      classId: mentee?.class?.id,
    })
    if (mentee?.profilePicture) {
      setPreviewProfilePicture(getImageURL(mentee?.profilePicture))
    }
  }, [form, mentee])
  ...
  async function onSubmit(values) {
    ...
    try {
      await updateMentee(updateMentorData).unwrap()
      form.reset()
      onOpenChange(false)
      Swal.fire({
        icon: "success",
        title: "Berhasil Update Mentee!",
        text: "Selamat Anda berhasil mengupdate mentee!",
      ...
      Swal.fire({
        icon: "error",
        title: "Gagal Update Mentee!",
        text: "Maaf, Anda gagal mengupdate mentee!",
        showConfirmButton: false,
        timer: 1500,
      })
    }
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="px-0 max-w-[600px] font-poppins">
        ...
        <AlertDialogFooter className=" px-8">
          ...
          <Button
            disabled={isLoadingUpdateMentee || !isFormValueChanged}
            form="update-mentee-form"
            type="submit"
            className="bg-green-500 hover:bg-green-600  gap-x-2 flex items-center">
            {isLoadingUpdateMentee && (
...
AdminEditMenteeDialog.propTypes = {
  open: PropTypes.bool,
